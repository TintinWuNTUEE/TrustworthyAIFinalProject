...
def normalize(img):
    ...
    return img
def pic(pic_l,pic_l_t,row,col,figname,title,filename=None,cmap=0):
    fig=plt.figure()
    for id,a in enumerate(pic_l):  
        ax = fig.add_subplot(row, col, id)
...
#filter=(1:wavelet),(2:jpeg)
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        match filter:
            ...
            case _:
                print("This is wrong filter type")
                return None
        ...
        if filter!=None and hog==1:
            ...
            hog_mask_rgb = mask_hog(data_A[0,:,:,:])
            hog_mask_rgb = normalize(hog_mask_rgb) #224,224,3
            hog_mask_rgb = np.transpose(hog_mask_rgb,(2,0,1))#3,224,224
            hog_mask_bar= 1-hog_mask_rgb
            # hog_mask_rgb=torch.tensor(normalize(hog_mask)).unsqueeze(0)#1,3,224,224
                
                
            # hog_mask = np.array([hog_mask]*3)
            # pic_all=[ ]#,img_hog_mask_rgb]
            # pic_all_t=[ 'hog_mask_rgb']#,'img_hog_mask_rgb']
            ...
            match filter:
                case 'wavelet':
                    ...
                    m0_HH_m1_HH = data_A_m1_HH+data_A_m0_HH #data_A_H
                    ...
                    pic_all_t=[ 'data_A',
                                ...
                                'm0_HH_m1_HH']
                case 'jpeg':
                    data_A_mask = hog_mask_rgb.unsqueeze(0)*data_A       #沒用到filter
                    data_A_bar_mask = hog_mask_bar.unsqueeze(0)*data_A
                    data_A_LL_m1=hog_mask_rgb*data_A_LL
                    data_A_LL_m0=hog_mask_bar*data_A_LL
                    data_A_HH_m1=hog_mask_rgb*data_A_HH
                    data_A_HH_m0=hog_mask_bar*data_A_HH
                    m0_1_LL_m1=normalize(data_A_bar_mask+data_A_LL_m1)
                    m0_1_HH_m1=normalize(data_A_bar_mask+data_A_HH_m1)

                    m0_0_1_m1=data_A_mask
                    m0_0_LL_m1=normalize(data_A_LL_m1)
                    m0_0_HH_m1=normalize(data_A_HH_m1)

                    LL_m0_1_m1=normalize(data_A_LL_m0+data_A_mask)
                    LL_m0_LL_m1=normalize(data_A_LL_m0+data_A_LL_m1)
                    LL_m0_HH_m1=normalize(data_A_LL_m0+data_A_HH_m1)

                    HH_m0_1_m1=normalize(data_A_HH_m0+data_A_mask)
                    HH_m0_LL_m1=normalize(data_A_HH_m0+data_A_LL_m1)
                    HH_m0_HH_m1=normalize(data_A_HH_m0+data_A_HH_m1)
                case _:
                    print("This is wrong filter type")
                    return None
            if  batch_idx==0:
                pic(pic_all,pic_all_t,5,3,figname='hogData.jpg')
            for id,a in enumerate(pic_all):
                if pic_all_t[id]!='data_A' and pic_all_t[id]!='hog_mask_rgb':
                    a=a.to(device)
                    # print(pic_all_t[id])
                    output = model(a)
                _, pred = output.max(1)
...
if __name__ == '__main__': 
    ...
    os.enfiron['CUDA_VISIBLE_DEVICES'] = args.gpu
    device = torch.device(f"cuda:{args.which_gpus}" if torch.cuda.is_available() else "cpu") 
